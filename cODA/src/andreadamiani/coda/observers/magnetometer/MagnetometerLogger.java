...
public class MagnetometerLogger extends Service implements
		SensorEventListener, Callback {

	private static final String VALUE_SEPARATOR = ";";

	private static final String DEBUG_TAG = "[cODA] MAGNETOMETER LOGGER";

	public MagnetometerLogger() {
		super();
	}
	...
	static class IncomingHandler extends Handler {
		...
		private final WeakReference<MagnetometerLogger> mService;

		IncomingHandler(MagnetometerLogger service) {
			mService = new WeakReference<MagnetometerLogger>(service);
		}
	...
	public void onCreate() {
		...
		if (android.os.Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.GINGERBREAD) {
			...
			String constantName = getResources().getString(
					R.string.magnetometer_sensor_sampling_rate);
			try {
			...
			sensorDelay = getResources().getInteger(
					R.integer.magnetometer_sensor_sampling_rate);
		}
		sensorSamplingRate = getResources().getInteger(
				R.integer.magnetometer_sensor_sampling_rate);
		samplingInterval = getResources().getInteger(
				R.integer.magnetometer_sensor_sampling_interval);
	}
	...
	public boolean handleMessage(Message msg) {
		...
		for (int i = 0; i < log.size(); i++) {
			...
			newValues.put(LogProvider.LOG_VALUE,
					MagnetometerLogger.valueToString(val));
			newValues
					.put(LogProvider.EXPIRY,
							System.currentTimeMillis()
									+ getResources().getInteger(
											R.integer.magnetometer_expiry));
