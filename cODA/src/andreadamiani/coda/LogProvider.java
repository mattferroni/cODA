...
public class LogProvider extends ContentProvider {
  ...
  public static final Uri CONTENT_URI = Uri.parse("content://andreadamiani.coda.log/obs");
  ...
  static {
   ...
   uriMatcher.addURI("andreadamiani.coda.log", 
                     "obs", ALLOBS);
   uriMatcher.addURI("andreadamiani.coda.log", 
           			"obs/*/last", LAST_READ);
   uriMatcher.addURI("andreadamiani.coda.log", 
...
//  public static final String KEY_ID = "_id";
  ...
  /**
   * Listing 8-9: Creating the Content Providers database
   */
    ...
    switch (uriMatcher.match(uri)) {
      case OLDEST_READ:
    	obsName = uri.getPathSegments().get(1);
      	queryBuilder.appendWhere(TIMESTAMP + "=" +createInnerQuery(obsName, min));
      	whereObsQuery(uri, queryBuilder, obsName);
      	break;
      case LAST_READ :
    	obsName = uri.getPathSegments().get(1);
    	queryBuilder.appendWhere(TIMESTAMP + "=" +createInnerQuery(obsName, max)); 
    	whereObsQuery(uri, queryBuilder, obsName);
    	break;
      case SINGLE_OBS : 
    	  whereObsQuery(uri, queryBuilder, obsName);
...
private String createInnerQuery(String obsName, String[] cols) {
	return SQLiteQueryBuilder.buildQueryString(true, MySQLiteOpenHelper.DATABASE_TABLE, cols, OBSERVER_NAME + "=" + obsName, null, null, null, null);
}
...
private void whereObsQuery(Uri uri, SQLiteQueryBuilder queryBuilder,
  ...
  public int delete(Uri uri, String selection, String[] selectionArgs) {
    ...
    switch (uriMatcher.match(uri)) {
      case OLDEST_READ : 
        ...
        selection = TIMESTAMP + "=" +createInnerQuery(obsName, cols) +" AND "+ OBSERVER_NAME + "=" + obsName;
        	break;  
      default: 
    	  throw new IllegalArgumentException("Partial deletion not implemented");
    }
  ...
  public Uri insert(Uri uri, ContentValues values) {
   ...
   db.insert(MySQLiteOpenHelper.DATABASE_TABLE, 
        null, values);
   
   while(true){
   Cursor cursor = query(Uri.parse(CONTENT_URI + values.getAsString(OBSERVER_NAME)+ "/oldest"), null, null, null, null);
   if(cursor.getCount()>0 && parseToTimestamp(cursor.getString(cursor.getColumnIndex(TIMESTAMP))) <getExpiryLimit()){
	   delete(Uri.parse(CONTENT_URI + values.getAsString(OBSERVER_NAME)+ "/oldest"), null, null);
   }else{
	   break;
   }
   }
...
private long getExpiryLimit() {
	return SystemClock.currentThreadTimeMillis()-getContext().getResources().getInteger(R.integer.logentry_lifetime);
}
